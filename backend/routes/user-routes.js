...
const editProfilePasswordValidation = require('../helpers/middlewares')
...
router.get('/profile-page/:id', (req, res, next) => {
  Promise.all([
    Users.findById(req.params.id).populate('followers'),
    Users.find({
      followers: req.params.id,
    }),
    Stories.find().populate('owner'),
  ])
    .then((resp) => {
      const user = resp[0]
      const following = resp[1]
      const userStory = resp[2].filter(
        (story) => req.params.id.localeCompare(story.owner._id) === 0,
      )
    ...
    Users.findById(req.params.id)
      .then((user) => {
        ...
        if (user.followers.includes(req.user._id)) {
          promise = Users.findByIdAndUpdate(
            req.params.id,
            {
        ...
        promise.populate('followers').then((resp) => {
          res.json(resp)
        })
      })
      .catch((err) => {
